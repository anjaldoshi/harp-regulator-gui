...
def test_validate_firmware_file(firmware_service, tmp_path):
    ...
    assert firmware_service.validate_firmware_file(str(tmp_path / "nonexistent.uf2")) is False
...
def test_inspect_firmware_cache(firmware_service, mocker):
    ...
    mocker.patch.object(firmware_service.cli, 'inspect_firmware', return_value=mock_info)
...
def test_check_firmware_compatibility(firmware_service):
    ...
    is_compatible = firmware_service.check_firmware_compatibility(device_id, firmware_version)
